<template>
  <div class="mt-4 flex flex-col sm:flex-row items-start sm:items-center sm:justify-between">
    ...
    <div class="w-full sm:w-auto text-end mt-2 sm:mt-0">
      <button v-if="savedTopic" @click="viewSavedTopic" class="text-sm font-medium text-end text-red-500">View Topic
        Ideas</button>
      <button v-if="!savedTopic" @click="viewSavedTopic" class="text-sm font-medium text-end text-red-500">Saved Topic
        Ideas</button>
    </div>
  </div>
  <TableLayout>
      ...
      <div class="py-2 overflow-x-auto scrollbar">
        <div class="relative overflow-x-auto">
          <template v-if="topicIdeas.length > 0">
            <table v-if="!savedTopic" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <!-- Table header -->
              ...
              <!-- Table body -->
              <tr v-for="(idea, index) in topicIdeas" :key="index"
                class="border-b border-gray-400 text-gray-500">
                <td class="px-4 py-3.5 text-[13px] whitespace-nowrap text-gray-800 font-medium">
                  {{ idea.topic }}
                </td>
                <td class="flex items-center justify-center gap-2 px-4 py-3.5 whitespace-nowrap">
                  <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-gray-800">{{ idea.keyword }}</span>
                </td>
                <td class="px-4 py-3.5 text-[13px] text-center text-gray-800 font-medium">{{ idea.topic_difficulty }}</td>
                <td class="px-4 py-3.5 text-[13px] text-center text-gray-800 font-medium">{{ idea.monthly_search_volume }}</td>
                <td class="px-4 py-3.5">
                  <button @click="contentExport(idea.topic)"
                    class="text-left text-[10px] text-[#304dc2] underline whitespace-nowrap font-medium">
                    Export to content generator
                  </button>
                </td>
                <td class="px-4 py-2">
                  <button @click="saveTopicIdeas(index)"
                    class="text-left text-[10px] underline whitespace-nowrap font-medium focus:outline-none">
                    <svg v-if="!isTopicSaved[index]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            ...
            <table v-if="savedTopic" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tr class="bg-[#414D61] text-white">
                ...
                <th class="py-2text-left whitespace-nowrap px-4 text-[13px] whitespace-nowrap font-medium text-center">
                  Keyword
                </th>
                <th class="px-4 py-2 whitespace-nowrap text-[13px] font-medium text-center">
                  Keyword difficulty
              ...
              <tr v-for="(idea, index) in topicIdeas" :key="index"
                ...
                <td class="flex items-center justify-center gap-2 px-4 py-2 whitespace-nowrap">
          ...
          <template v-else>
            <p class="text-[13px] text-red-600">Sorry! Unable to produce topic ideas. Unclear channel earlier uploads. {{
              errorMsg }}</p>
          </template>
        </div>
      </div>
